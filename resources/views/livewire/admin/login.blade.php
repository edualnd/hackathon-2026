...
<body
    class="min-h-screen font-body antialiased"
    style="background: linear-gradient(135deg, #0c4646 0%, #117d7d 45%, #97bc10 100%)"
>
    <div class="flex min-h-screen items-center justify-center px-4 py-10">
        <div class="w-full max-w-md">
            <div class="mb-6 flex items-center justify-center gap-2.5">
                <span
                    class="relative flex size-11 items-center justify-center rounded-xl bg-action-primary"
                >
            ...
            <div
                class="rounded-2xl border border-white/25 bg-white/12 p-8 shadow-2xl backdrop-blur-xl"
            >
                ...
                @if ($errors->any())
                    <div
                        class="mt-4 rounded-xl bg-feedback-danger px-4 py-3 font-body text-sm text-text-on-danger"
                    >
                ...
                <form wire:submit.prevent="entrar" class="mt-6 space-y-4">
                    <div>
                        <label
                            for="email"
                            class="mb-1.5 block font-body text-sm font-medium text-white/90"
                        ...
                        <input
                            ...
                            type="email"
                            wire:model="email"
                            placeholder="nome@educacaocaraguatatuba.sp.gov.br"
                        ...
                        />
                        @error ('email')
                            <p class="mt-1.5 font-body text-xs text-amber-200">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <p class="mt-5 text-center font-body text-xs text-white/60">Acesso restrito à equipe da Secretaria de Educação. Ambiente de demonstração — sem autenticação real.</p>
            </div>

            <p class="mt-6 text-center font-body text-xs text-white/70">
                <a href="{{ route('site.search') }}" class="hover:underline"
                    >← Voltar para a área dos pais</a
                >
            </p>
        </div>
    </div>
