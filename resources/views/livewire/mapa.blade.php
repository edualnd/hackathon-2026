<div>
    ...
    <div
        id="map"
        class="relative isolate z-0 h-[80dvh] w-full rounded-2xl sm:h-[400px] lg:h-[520px]"
        wire:ignore
    ></div>
</div>
    ...
    document.addEventListener('livewire:init', () => {
        ...
        function desenharMarcadores(escolas) {
            ...
            escolas.forEach((escola) => {
                ...
                if (totalVagas >= 31) {
                    icon = greenIcon;
                } else if (totalVagas >= 20) {
                    icon = yellowIcon;
                } else {
                    icon = redIcon;
                }

                const marker = L.marker([escola.lat, escola.lng], { icon }).addTo(marcadoresLayer); // 2. adiciona no grupo, não direto no map
