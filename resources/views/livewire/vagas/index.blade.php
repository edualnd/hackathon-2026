            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="border-b border-seduc-neutral-200 bg-seduc-neutral-100/60">
                        <th class="px-5 py-3 font-body text-[11px] font-semibold uppercase tracking-wide text-seduc-neutral-600">Escola</th>
                ...
                <tbody class="divide-y divide-seduc-neutral-100">
                    @forelse ($vagas as $vaga)
                        <tr wire:key="vaga-{{ $vaga->id }}" class="hover:bg-seduc-neutral-100/40">
                            ...
                            <td class="px-5 py-4 font-data text-sm text-text-on-surface">{{ $vaga->qtd }}</td>
                            <td class="px-5 py-4 font-data text-sm text-seduc-neutral-600">{{ $vaga->matriculados_count ?? 0 }}</td>
                            <td class="px-5 py-4">
                                @php $disponiveis = max($vaga->qtd - ($vaga->matriculados_count ?? 0), 0); @endphp
                                @if ($disponiveis > 0)
                                    <x-site.badge variant="success">{{ $disponiveis }} {{ $disponiveis === 1 ? 'vaga livre' : 'vagas livres' }}</x-site.badge>
                                @else
                            ...
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <button type="button" wire:click="$dispatch('dispatchOpenModalEditVaga', { id: {{ $vaga->id }} })"
                                        class="flex size-8 items-center justify-center rounded-lg border border-seduc-neutral-200 text-seduc-neutral-600 hover:border-teal-dark-400 hover:text-teal-dark-600" title="Editar">
                                    ...
                                    <button wire:click="excluir({{ $vaga->id }})" wire:confirm="Remover a vaga de {{ $vaga->serie }} em {{ $vaga->escola?->nome }}? Esta ação não pode ser desfeita."
        ...
        @if ($vagas->total() > 0)
            <div class="flex flex-col gap-3 border-t border-seduc-neutral-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="font-body text-xs text-seduc-neutral-600">
                    Mostrando {{ $vagas->firstItem() }} até {{ $vagas->lastItem() }} de {{ $vagas->total() }} resultados
                </p>
