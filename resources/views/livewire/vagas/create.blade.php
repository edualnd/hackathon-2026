<div>
    <x-modal wire:model="show" maxWidth="lg">
        <div class="px-6 py-6">
            ...
            <h2 class="mt-0.5 font-heading text-lg font-semibold text-text-on-surface">
                Nova vaga
            </h2>
            ...
            <form wire:submit.prevent="salvar" class="mt-5 space-y-4">
                <div>
                    <x-site.select
                        label="Escola"
                        name="vaga_create_escola_id"
                        wire:model="vaga.escola_id"
                    >
                ...
                <div>
                    ...
                    @error ('vaga.serie')
                        <p class="mt-1.5 font-body text-xs text-text-on-danger">{{ $message }}</p>
                    @enderror
                </div>

                <x-site.input
                    label="Quantidade de vagas"
                    name="vaga_create_qtd"
                    type="number"
                    min="0"
                    wire:model="vaga.qtd"
                    :error="$errors->first('vaga.qtd')"
                />

                <div class="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
                    <x-site.button type="button" variant="ghost" wire:click="$set('show', false)">
                        Cancelar</x-site.button
                    >
                    <x-site.button
                        type="submit"
                        variant="primary"
