<div class="mx-auto max-w-7xl px-6 py-8 sm:px-10">
    ...
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        @if (Auth::user()->escola_id)
            <x-site.button variant="primary" href="{{ route('alunos.create') }}">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="lg:col-span-2">
                <x-site.input
                    ...
                    placeholder="Buscar por nome, RA ou certidão"
                />
            </div>
            <x-site.select
    ...
    >
                ...
                <tbody class="divide-y divide-seduc-neutral-100">
                    @forelse ($alunos as $aluno)
                        <tr wire:key="aluno-{{ $aluno->id }}" class="hover:bg-seduc-neutral-100/40">
                            ...
                            <td class="px-5 py-4 font-body text-sm text-seduc-neutral-600">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    @if (!Auth::user()->escola_id)
                                        <button
                                            wire:click="excluir({{ $aluno->id }})"
                                            wire:confirm="Remover o cadastro de {{ $aluno->nome }}? Esta ação não pode ser desfeita."
        ...
        @if ($alunos->total() > 0)
            ...
            >
                <p class="font-body text-xs text-seduc-neutral-600">Mostrando {{ $alunos->firstItem() }} até {{ $alunos->lastItem() }} de {{ $alunos->total() }} resultados</p>
