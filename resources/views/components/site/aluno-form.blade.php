...
<form wire:submit.prevent="salvar" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <x-site.input
                    label="Nome completo"
                    name="nome"
                    wire:model="dadosAluno.nome"
                    placeholder="Nome do estudante"
                    :error="$errors->first('dadosAluno.nome')"
                />
            </div>
            <x-site.input
                label="RA (com dígito)"
                name="ra"
                wire:model="dadosAluno.ra"
                placeholder="999.999.999-9"
                :error="$errors->first('dadosAluno.ra')"
            />

            <x-site.input
                label="CPF"
                name="cpf"
                wire:model="dadosAluno.cpf"
                placeholder="999.999.999-99"
                :error="$errors->first('dadosAluno.cpf')"
            />
            ...
            <x-site.input
                ...
                type="date"
                wire:model="dadosAluno.data_nascimento"
                :error="$errors->first('dadosAluno.data_nascimento')"
            />
            ...
            <div class="lg:col-span-2">
                <x-site.input
                    ...
                    wire:model="dadosAluno.certidao_nascimento"
                    placeholder="000000.00.0000.0.00000.000.0000000"
                    :error="$errors->first('dadosAluno.certidao_nascimento')"
                />
            </div>
            <x-site.input
                label="Escola"
                name="escola"
                readonly
                value="{{ $vagas[0]['escola'] }}"
                placeholder="{{ $vagas[0]['escola'] }}"
                :error="$errors->first('dadosAluno.escola_id')"
            />

            @error ('dadosAluno.vaga_id')
                <p class="-mt-3 font-body text-xs text-text-on-danger">{{ $message }}</p>
            @enderror
            ...
            @endif
            <x-site.select
                label="Escola / série pretendida"
                name="vaga_id"
                wire:model="dadosAluno.vaga_id"
            >
                <option value="">Selecione...</option>
    ...
    <div class="rounded-2xl border border-seduc-neutral-200 bg-background-surface p-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <x-site.input
                ...
                wire:model="dadosAluno.nome_responsavel"
                placeholder="Nome completo"
                :error="$errors->first('dadosAluno.nome_responsavel')"
            />
            <x-site.input
                label="CPF do responsável"
                name="cpf_responsavel"
                wire:model="dadosAluno.cpf_responsavel"
                placeholder="999.999.999-99"
                :error="$errors->first('dadosAluno.cpf_responsavel')"
            />
            <x-site.input
                label="Parentesco"
                name="parentesco"
                wire:model="dadosAluno.parentesco"
                placeholder="Mãe, pai, avó..."
                :error="$errors->first('dadosAluno.parentesco')"
            />
            <x-site.input
                label="Telefone pessoal"
                name="telefone_pessoal"
                wire:model="dadosAluno.telefone_pessoal"
                placeholder="(00) 00000-0000"
                :error="$errors->first('dadosAluno.telefone_pessoal')"
            />
            <x-site.input
                label="Telefone para recado"
                name="telefone_recado"
                wire:model="dadosAluno.telefone_recado"
                placeholder="(00) 00000-0000"
            />
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-seduc-neutral-200 bg-background-surface p-6">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            <x-site.input
                label="CEP"
                name="cep"
                wire:model.live="dadosAluno.cep"
                placeholder="00000-000"
                :error="$errors->first('dadosAluno.cep')"
            />
            <x-site.input
                label="UF"
                name="uf"
                wire:model="dadosAluno.uf"
                maxlength="2"
                :error="$errors->first('dadosAluno.uf')"
            />
            <div class="col-span-2">
                <x-site.input
                    label="Município"
                    name="municipio"
                    wire:model="dadosAluno.municipio"
                    :error="$errors->first('dadosAluno.municipio')"
                />
            </div>
            <div class="col-span-2">
                <x-site.input
                    label="Bairro"
                    name="bairro"
                    wire:model="dadosAluno.bairro"
                    :error="$errors->first('dadosAluno.bairro')"
                />
            </div>
            <div class="col-span-2 sm:col-span-2 lg:col-span-4">
                <x-site.input
                    label="Logradouro"
                    name="logradouro"
                    wire:model="dadosAluno.logradouro"
                    :error="$errors->first('dadosAluno.logradouro')"
                />
            </div>
    ...
    <div class="rounded-2xl border border-seduc-neutral-200 bg-background-surface p-6">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            @foreach ([
                ...
                <label
                    class="flex cursor-pointer items-start gap-3 rounded-xl border border-seduc-neutral-200 p-4 hover:border-teal-dark-400"
                >
                    <input
                        type="checkbox"
                        wire:model="dadosCriterio.{{ $campo }}"
                        class="mt-0.5 size-4 rounded border-seduc-neutral-400 text-teal-dark-600 focus:ring-teal-dark-400"
                    />
    ...
    <div class="rounded-2xl border border-seduc-neutral-200 bg-background-surface p-6">
        ...
        <textarea
            id="observacao"
            wire:model="dadosAluno.observacao"
            rows="3"
            placeholder="Alguma informação adicional relevante para a análise..."
            class="w-full rounded-xl border border-seduc-neutral-300 bg-background-surface px-4 py-3 font-body text-sm text-text-on-surface placeholder:text-seduc-neutral-400 focus:outline-none focus:ring-2 focus:ring-teal-dark-400 focus:border-teal-dark-500"
        ></textarea>
    </div>
    @if ($errors->any())
        <div class="mb-6 rounded-lg border border-red-300 bg-red-50 p-4">
            <h3 class="mb-2 font-semibold text-red-700">Foram encontrados os seguintes erros:</h3>
